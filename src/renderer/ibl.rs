...
impl Probe {
    pub fn new(path: &str) -> Result<Self> {
        timed_scope("HDR map loading", || load_hdr_image(path))?;

        let equimap = load_hdr_image(path)?;
        let equi_tex_id = Self::create_equi_texture(equimap);
        let cubemap_tex_id = Self::equi_to_cubemap(equi_tex_id)?;
    ...
    fn brdf_integration() -> Result<u32, eyre::ErrReport> {
    ...
    fn compute_prefilter_map(cubemap_tex_id: u32) -> Result<u32, eyre::ErrReport> {
