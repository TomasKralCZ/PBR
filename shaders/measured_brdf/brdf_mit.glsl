...
vec3 lookup_brdf_mit(vec3 toLight, vec3 toViewer, vec3 normal, vec3 tangent, vec3 bitangent)
{
    ...
    int redIndex
        = i_phi_in + phi_in_dim * (i_phi_diff + phi_diff_dim * (i_theta_out + theta_out_dim * i_theta_in));
    ...
    float r = mitBrdfTable[redIndex];
    float g = mitBrdfTable[greenIndex];
    float b = mitBrdfTable[blueIndex];

    if (r < 0 || g < 0 || b < 0) {
        return vec3(0, 0, 0);
    } else {
        return 10.0 * vec3(r, g, b);
    }
}
